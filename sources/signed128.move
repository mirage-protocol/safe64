...
module safe64::signed128 {
    ...
    /// representation.
    ///
    /// To consume these values, first call `get_is_negative()` to determine if the Signed128
    ...
    struct Signed128 has copy, drop, store {
        negative: bool,
        magnitude: u128,
    }

    public fun zero(): Signed128 {
        new(0, false)
    }
    ...
    public fun new_from_64(magnitude: u64, negative: bool): Signed128 {
        ...
        Signed128 {
            magnitude: (magnitude as u128),
            negative: negative,
        }
    }

    public fun new(magnitude: u128, negative: bool): Signed128 {
        // Ensure we have a single zero representation: (0, false).
        // (0, true) is invalid.
        if (magnitude == 0) {
            negative = false;
    ...
    // adds two Signed128s, returns a new one
    public fun add(a: Signed128, b: Signed128): Signed128 {
    ...
    public fun sub(a: Signed128, b: Signed128): Signed128 {
        b.negative = !b.negative;
        add(a, b)
    }

    public fun get_is_negative(i: &Signed128): bool {
        i.negative
    }

    public fun get_magnitude_if_positive(in: &Signed128): u128 {
    ...
    #[test]
    #[expected_failure(abort_code = 4501, location = safe64::signed128)]
    ...
    #[test]
    #[expected_failure(abort_code = 4502, location = safe64::signed128)]
    ...
    #[test]
    fun test_get_is_negative_from_64() {
        assert!(get_is_negative(&new_from_64(234, true)) == true, 1);
        assert!(get_is_negative(&new_from_64(767, false)) == false, 1);
    }

    #[test]
    fun test_get_magnitude_if_positive_positive_from_64() {
        assert!(get_magnitude_if_positive(&new_from_64(7686, false)) == 7686, 1);
    }

    #[test]
    #[expected_failure(abort_code = 4501, location = safe64::signed128)]
    fun test_get_magnitude_if_positive_negative_from_64() {
        assert!(get_magnitude_if_positive(&new_from_64(7686, true)) == 7686, 1);
    }

    #[test]
    fun test_get_magnitude_if_negative_negative_from_64() {
        assert!(get_magnitude_if_negative(&new_from_64(7686, true)) == 7686, 1);
    }

    #[test]
    #[expected_failure(abort_code = 4502, location = safe64::signed128)]
    fun test_get_magnitude_if_negative_positive_from_64() {
        assert!(get_magnitude_if_negative(&new_from_64(7686, false)) == 7686, 1);
    }
